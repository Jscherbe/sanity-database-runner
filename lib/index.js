...
export function createDatabaseRunner(config) {
  ...
  const { client, dataset, backup: performBackup = true, force = false } = config;

  // Merge default and user-configured paths
  const paths = { ...defaultPaths, ...config.paths };

  // Resolve all paths relative to the CWD
  const rootDir = paths.cwd;
  const resolvedPaths = {
    updates: path.resolve(rootDir, paths.updates),
    backups: path.resolve(rootDir, paths.backups),
    sanityBin: paths.sanityBin // This is resolved later by `which` if not provided
  };
  ...
  async function run(scriptName) {
    ...
    const scriptPath = path.join(resolvedPaths.updates, `${scriptName}.js`);
    try {
