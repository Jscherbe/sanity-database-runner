...
export function createDatabaseRunner(config) {
  ...
  const { client, paths, backup: performBackup = true } = config;

  if (!client) {
    throw new Error("Database runner requires a sanity `client` instance in config.");
  }
  ...
  async function run(scriptName) {
    ...
    const response = await prompts({
      type: 'confirm',
      name: 'value',
      message: `Are you sure you want to run the script "${scriptName}" on the "${client.config().dataset}" dataset?`,
      initial: false
    });
